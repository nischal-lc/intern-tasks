	...
	const addData = async (e) => {
		...
		try {
			...
			toast(response.data.message, {
				...
				theme: "light",
				className:
					"bg-white shadow-md rounded-md p-3 border border-[hsla(0,0%,0%,0.1)]",
			...
			toast(err.data.message, {
				...
				theme: "light",
				className:
					"bg-red-600/10 shadow-md rounded-md p-3 border border-[hsla(0,0%,0%,0.1)]",
				bodyClassName: "text-sm",
			});
		}
	};
...
const Card = ({ country, magnitude, date, flag, id, setRefresh, refresh }) => {
	const removeData = async (id) => {
		try {
			await axios.delete(`/api/earthquakes/${id}`, {
				headers: {
					"Content-Type": "application/json",
				},
			});
	...
	const editData = async (e) => {
		...
		try {
			...
			toast(response.data.message, {
				...
				theme: "dark",
				className:
					"bg-neutral-900 shadow-md text-white rounded-md p-3 border border-[hsla(0,0%,0%,0.1)]",
				bodyClassName: "text-sm",
				progressClassName: "!h-4 !bg-green-700",
			});
			setRefresh(!refresh);
		} catch (err) {
			toast(err.data.message, {
		...
		<div className='flex flex-col p-2 bg-gray-200 max-h-max transition duration-200 rounded-md shadow-md shadow-black/10 border border-[hsla(0,0%,0%,0.2)]'>
			<div className='flex justify-between'>
				<div className='flex gap-2 items-center'>
					<img src={flag} className='size-5 select-none' />
